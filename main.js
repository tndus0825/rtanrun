...
const rtan = {
  ...
  draw() {      // 달리는 애니메이션 구현
    if (gameOver) {
      ...
      // 달리는 애니메이션 구현
      // timer를 60으로 나눈 나머지가 30보다 클때 A, 작을 때 B
      if (timer % 60 > 30) {
...
const OBSTACLE_FREQUENCY = 60; // 장애물 생성 빈도
const OBSTACLE_SPEED = 4; // 장애물 이동 속도

/** 장애물 클래스 정의 */
// 장애물은 여러개, 하나의 객체를 여러 개 만들어주는게 클래스
class Obstacle {
  constructor() {
    this.x = canvas.width;
    this.y =
    // y값을 랜덤으로 하여 장애물이 랜덤으로 날아옴
...
/** 3-1 배경 화면 그리기 */
//배경화면은 같은 화면이 계속 반복적으로 지나가 연속되는 것처럼 만들어 함수를 반복적으로 이어나감
...
//Promise 자바스크립트 문법
//약속하다, 얘가 로드되면 OK 약속해줘 라는 뜻
//두 개의 이미지가 로드가 도면 오케이 해달라는 뜻
...
function animate() {
  ...
  if (timer % OBSTACLE_FREQUENCY === 0) {
    //장애물 새로 생성
      //리스트, 장애물 주머니에 다 넣어놓고 canvous에 보여준다,
  ...
  obstacleArray.forEach((obstacle) => {
      ...
      //주머니 비우는 역할, 벽에 닿음녀 지워줘라
      if (obstacle.x < -OBSTACLE_WIDTH) { //벽에 닿으면
      ...
      // 충돌 검사
      //르탄이랑 장애물 만나면 실행되는 함수, 장애물과 만난다면
...
//animate(); 얘가 여기 있으면 시작화면 안뜨고 바로 애니메이션 실행됨, 강제로 실행시키는거라
/** end of 게임 애니메이션 */
...
function restartGame() {
  ...
  scoreText.innerHTML = "현재점수: " + score;
  // 게임 오버 시 르탄이 위치 초기화
  //위치 초기화 안하면 게임오버된 곳에서 게임 다시 시작함
  rtan.x = 10;
  rtan.y = 400;
  animate();
}
